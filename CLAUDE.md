...
- **Do not re-encode the hero videos with ffmpeg.** Both WebM files carry a transparent background (`alpha_mode=1`, stored as per-frame `BlockAdditional` data, not in the pixel format). ffmpeg's vp9 decoder cannot read WebM alpha — it declares no `yuva` pixel formats — so it drops the channel silently and copies the `ALPHA_MODE` tag anyway, producing a file that claims transparency but renders with a solid background. `ffprobe`'s `pix_fmt` says `yuv420p` either way and is not a reliable check; count `BlockAdditions` in the container instead (~130 when alpha is present, ~5 when it is gone). The VP8 file is not a redundant fallback — it is the alpha fallback for browsers without VP9 alpha support. Re-encoding would have to start from a source that still has its alpha, not from these files.
- Remaining page weight is therefore almost entirely the hero: `hero-video-vp8.webm` (1.7 MB) and `hero-video.webm` (1.2 MB). The poster is already an optimised WebP (38 KB, alpha preserved).
