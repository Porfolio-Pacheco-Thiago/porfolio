        ...
        <nav className={`riel ${abierto ? 'is-abierto' : ''}`} aria-label={t('nav.contact')}>
            <div className="riel-caja" id="riel-contacto">
                ...
                <ul className="riel-lista">
                    {socialsActivos.map((social, i) => (
                        <li key={social.id}>
                            {/* `nombraPorAria` porque plegado el nombre está recortado y
                                no se ve: si el nombre accesible saliera del texto
                                visible, el enlace no tendría ninguno. */}
                            <EnlaceSocial
                                social={social}
                                className="riel-link"
                                nombraPorAria
